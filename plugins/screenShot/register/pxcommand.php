...
class pxplugin_screenShot_register_pxcommand extends px_bases_pxcommand{
	...
	/* screenShot 拡張項目 */
	private $multiDevice_user_agent_list = array(
		'sp'=>'iPhone',
		'tb'=>'iPad',
	);
...
function contEditPublishTargetPathApply(formElm){
	...
	window.location.href = path + '?PX=plugins.screenShot&path_target='+encodeURIComponent(path);
}
</script>
<?php
		$src .= ob_get_clean();
		if(!is_dir($this->path_docroot_dir)){
			...
			$src .= '</ul>';
			$src .= '<p>この設定は、mainconf.ini の plugin-screenShot.ua の項目で変更できます。</p>'."\n";
			$src .= '	<div class="code"><pre><code>[plugin-screenShot]'."\n";
			$src .= '; プラグイン &quot;screenShot&quot; 拡張項目'."\n";
			$src .= 'ua = &quot;'."\n";
			$src .= 'sp=iPhone'."\n";
			$src .= 'tb=iPad'."\n";
			$src .= '&quot;;'."\n";
			...
			$src .= '	<div><input type="hidden" name="PX" value="plugins.screenShot.run" />'.(strlen( $this->param_path_target )?'<input type="hidden" name="path_target" value="'.t::h( $this->path_target ).'" />':'').'</div>'."\n";
			$src .= '	</form>'."\n";
			$src .= '</div><!-- /.unit -->'."\n";
			...
			$src .= '       	<dd>$ curl '.t::h($auth_curl).''.t::h( t::data2text('http://'.$_SERVER['HTTP_HOST'].$this->path_target.'?PX=plugins.screenShot.run'.(strlen( $this->param_path_target )?'&path_target='.urlencode( $this->path_target ):'') ) ).'</dd>'."\n";
			$src .= '		<dt>"wget" コマンドが使える場合</dt>'."\n";
			$src .= '       	<dd>$ wget '.t::h($auth_wget).''.t::h( t::data2text('http://'.$_SERVER['HTTP_HOST'].$this->path_target.'?PX=plugins.screenShot.run'.(strlen( $this->param_path_target )?'&path_target='.urlencode( $this->path_target ):'') ) ).'</dd>'."\n";
			$src .= '   </dl>'."\n";
			$src .= '</div><!-- /.topic_box -->'."\n";
			$src .= ''."\n";
	...
	private function setup(){
		// multhUAPublish 拡張
		$plugin_conf_ua = $this->px->get_conf('plugin-screenShot.ua');
			...
			case 'http':
				...
				if( $extension == 'html' ){
					...
					foreach( $this->multiDevice_user_agent_list as $device_type_id=>$user_agent_string ){
						...
						$this->publish_single_dynamic_contents( $url, $tmp_path, $user_agent_string.' ('.$this->crawler_user_agent.')' );
						if( md5_file($this->path_tmppublish_dir.'/htdocs/'.$path) == md5_file($tmp_path) ){
							$this->px->dbh()->rm( $tmp_path );
							print '    same content!'."\n";
						}
					}
				}
	...
	/**
	 * screenShotカスタム：単体の動的コンテンツをパブリッシュする
	 */
	private function publish_single_dynamic_contents($url, $path, $user_agent){
		...
		$httpaccess->set_user_agent( $user_agent );//HTTP_USER_AGENT
		...
		$this->px->dbh()->mkdir_all( dirname($path) );
		$httpaccess->save_http_contents( $path );//ダウンロードを実行する
	...
	/**
	 * screenShotカスタム：単体の動的コンテンツのスクリーンショットを作成する
	 */
	private function screenshot_single_dynamic_contents($url){
		...
		$path_script = $this->px->dbh()->get_realpath( $this->px->get_conf('paths.px_dir').'plugins/screenShot/libs/capybara/capybaraWebkit.rb' );
...
<?php
		...
		$this->px->dbh()->copy(
			$this->px->dbh()->get_realpath( $this->px->get_conf('paths.px_dir').'plugins/screenShot/libs/capybara/_capybaraWebkit.rb' ),
			$this->path_tmppublish_dir.'/screenShot/_cmd/_capybaraWebkit.rb'
		);

		$this->px->dbh()->copy(
			$this->px->dbh()->get_realpath( $this->px->get_conf('paths.px_dir').'plugins/screenShot/libs/phantom/_phantom_capture.js' ),
